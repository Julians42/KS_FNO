...
def TKE(u):
    ...
    return np.mean((u-umean)**2, axis=(0, 2))
...
def plot_histogram_density(data_result, nn_result):
    data_values = data_result[:, 500:, :].flatten()
    nn_values = nn_result[:, 500:, :].flatten()
    res = nn_result.shape[0]
    # filter out nans and infs from nn_values
    nn_values = nn_values[np.isfinite(nn_values)]
    nn_values = nn_values[np.abs(nn_values) < 5]
    ...
    plt.title(f"Empirical Density Comparison at res {res} with KS: {ks_stat.statistic:.2g}, p-value: {ks_stat.pvalue:.2g}")
    return fig, ks_stat


def plot_tke_spectra(data_result, nn_result, threshold = 3.2):
    data_result = data_result[:, 500:, :]
    nn_result = nn_result[:, 500:, :]
    x_vals = np.linspace(0, threshold, 500)
    res = nn_result.shape[0]
    # filter out nans and infs from nn_values
    # nn_result = nn_result[np.isfinite(nn_result)]
    # nn_result = nn_result[np.abs(nn_result) < 5]
    ...
    plt.title(f"TKE Density Comparison at res {res} with KS: {ks_stat.statistic:.2g}, p-value: {ks_stat.pvalue:.2g}")
    return fig, ks_stat
...
def plot_spatial_autocorr(data_result, nn_result, config):
    ...
    data_result = data_result[:, 500:, 0]
    nn_result = nn_result[:, 500:, 0]

    l = config.data.architecture.l
    x_values = np.linspace(0, l, data_result.shape[0])
    x_values_nn = np.linspace(0, l, nn_result.shape[0])
    ...
    plt.plot(x_values_nn, np.mean(nn_autocorr_list, axis=1), label='FNO', color='r', linestyle = '--')
...
def plot_trajectory_comparison(data_result, nn_result, config):
    ...
    print(nn_result[:, :, 0].shape)
    print(t_vals.shape)
    print(n_points // coarsen_factor)
    ax[1].pcolormesh(t_vals,
                    x_vals,
                    #np.linspace(0, l, nn_result.shape[0]),
                    nn_result[:, :, 0],
                    vmin = -3,
                    vmax = 3,
                    cmap = "inferno")
    ax[1].set_title(f"FNO at res {len(x_vals)} (starting at t=500 * {dt})")
...
def plot_temporal_autocorr(data_result, nn_result, config):
    ...
    total_time = config.data.architecture.T * config.data.architecture.dt
...
def evaluate_metrics(true_data, nn_data, config, save_to_pdf=False, tke_plot_max_threshold = 3.2):
    print(f"Evaluating metrics for data with shape {true_data.shape} and {nn_data.shape}")
    fig0 = plot_trajectory_comparison(true_data, nn_data, config)
    print("plotted 1")
    fig1, ks_density = plot_histogram_density(true_data[:, :, 0:2], nn_data[:, :, 0:2]);
    print("plotted 2")
    ...
    print("plotted 3")
    fig4 = plot_spatial_autocorr(true_data, nn_data, config)
    ...
    if save_to_pdf != False:
        os.makedirs(save_to_pdf, exist_ok=True)
        fig0.savefig(f'{save_to_pdf}/trajectory_comparison.png')
        fig1.savefig(f'{save_to_pdf}/histogram_density.png')
        #fig2.savefig(f'{save_to_pdf}/tke_spectra.png')
        fig3.savefig(f'{save_to_pdf}/freq_spectrum.png')
        fig4.savefig(f'{save_to_pdf}/spatial_autocorr.png')
    #    fig5.savefig(f'{save_to_pdf}/temporal_autocorr.png')
    else:
        return fig0, fig1,  fig3, fig4, ks_density#, ks_tke, #fig5fig2,
